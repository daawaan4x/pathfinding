<script lang="ts">
	import { Eraser, Pen, Play, Undo } from "lucide-svelte";
	import GridCanvas from "./grids/[id]/GridCanvas.svelte";
	import type { PointerType } from "./grids/[id]/GridCanvasHelpers";
	import { GridNode, type GridRecord } from "$lib/types/grid-service";
	import ToggleGroup from "$lib/shadcn/components/ui/toggle-group/toggle-group.svelte";
	import ToggleGroupItem from "$lib/shadcn/components/ui/toggle-group/toggle-group-item.svelte";
	import * as Tooltip from "$lib/shadcn/components/ui/tooltip";
	import Button from "$lib/shadcn/components/ui/button/button.svelte";
	import { toast } from "svelte-sonner";
	import { AStar } from "$lib/pathfinding/a-star";
	import Input from "$lib/shadcn/components/ui/input/input.svelte";

	let size: number = $state(10);

	let pointer: PointerType | undefined = $state("pen");
	let astar: AStar | undefined = $state();
	let grid: GridRecord | undefined = $state();
	reset();

	function reset() {
		grid = {
			id: "",
			name: "Playground",
			size,
			tags: [],
			created_at: new Date(),
			updated_at: new Date(),
			data: new Array<GridNode>(size * size).fill(GridNode.empty),
		};

		astar = undefined;
	}

	function run() {
		const fps = 24;
		if (!grid) return;

		astar = new AStar(grid.size, grid.data);
		const id = setInterval(function interval() {
			if (!astar) return clearInterval(id);
			if (astar.status == "finished") toast.success("Finished Simulation.");
			if (astar.status == "no-solution") toast.error("No Solution Found.");
			if (astar.status == "finished" || astar.status == "no-solution") return clearInterval(id);
			astar.step();
		}, 1000 / fps);
	}
</script>

<div class="relative flex min-h-screen flex-col">
	<header class="relative flex justify-center pt-8">
		<div class="flex flex-col items-center pr-11">
			<h1
				class="flex flex-row gap-3 bg-gradient-to-b from-black/70 to-black bg-clip-text pb-4 text-6xl font-extrabold leading-none tracking-tight text-transparent">
				Pathfinding
			</h1>
			<blockquote class="flex flex-row items-center space-x-2 opacity-60">
				<p class="text-gray-700">
					by <i> Theone Genesis Eclarin </i>
				</p>
				<div class="">
					<a href="https://github.com/daawaan4x">
						<img
							class="rounded border"
							src="https://img.shields.io/badge/@daawaan4x-white?logo=github&logoColor=121013"
							alt="Github" />
					</a>
				</div>
			</blockquote>
		</div>
	</header>

	<main class="relative flex flex-1 flex-row items-center justify-center space-x-3">
		<GridCanvas bind:pointer bind:data={grid} bind:astar size={550} />

		<section class="space-y-12">
			<ToggleGroup type="single" class="flex-col rounded-md border p-1" bind:value={pointer}>
				<Tooltip.Root>
					<Tooltip.Trigger>
						<ToggleGroupItem class="px-[6px]" value="pen">
							<Pen
								strokeWidth={pointer == "pen" ? 3 : 2}
								class={pointer == "pen" ? "black" : "text-muted-foreground"} />
						</ToggleGroupItem>
					</Tooltip.Trigger>
					<Tooltip.Content side="right">Add Block</Tooltip.Content>
				</Tooltip.Root>
				<Tooltip.Root>
					<Tooltip.Trigger>
						<ToggleGroupItem class="px-[6px]" value="eraser">
							<Eraser
								strokeWidth={pointer == "eraser" ? 3 : 2}
								class={pointer == "eraser" ? "black" : "text-muted-foreground"} />
						</ToggleGroupItem>
					</Tooltip.Trigger>
					<Tooltip.Content side="right">Erase</Tooltip.Content>
				</Tooltip.Root>
			</ToggleGroup>

			<div class="flex flex-col space-y-1 rounded-md border p-1">
				<Tooltip.Root>
					<Tooltip.Trigger>
						<Input
							class="hide-arrow aspect-square p-1 text-center"
							type="number"
							bind:value={size}
							on:keydown={(e: KeyboardEvent) => {
								if (e.key != "Enter") return;
								size = Math.max(10, Math.min(30, size));
								reset();
							}}
							on:blur={() => {
								size = Math.max(10, Math.min(30, size));
							}} />
					</Tooltip.Trigger>
					<Tooltip.Content side="right">Grid size (10-30)</Tooltip.Content>
				</Tooltip.Root>

				<Button class="px-[6px]" on:click={reset}>
					<Tooltip.Root>
						<Tooltip.Trigger>
							<Undo />
						</Tooltip.Trigger>
						<Tooltip.Content side="right">Reset Grid</Tooltip.Content>
					</Tooltip.Root>
				</Button>
			</div>

			<div class="flex-col rounded-md border p-1">
				<Button class="px-[6px]" on:click={run}>
					<Tooltip.Root>
						<Tooltip.Trigger>
							<Play />
						</Tooltip.Trigger>
						<Tooltip.Content side="right">Run Algorithm</Tooltip.Content>
					</Tooltip.Root>
				</Button>
			</div>
		</section>
	</main>

	<a
		href="https://github.com/daawaan4x/pathfinding"
		target="_blank"
		class="github-corner fixed right-0 top-0 h-[80px] w-[80px]"
		aria-label="View source on GitHub"
		><svg
			width="80"
			height="80"
			viewBox="0 0 250 250"
			style="fill:#151513; color:#fff; position: absolute; top: 0; border: 0; right: 0;"
			aria-hidden="true"
			><path d="M0,0 L115,115 L130,115 L142,142 L250,250 L250,0 Z" /><path
				d="M128.3,109.0 C113.8,99.7 119.0,89.6 119.0,89.6 C122.0,82.7 120.5,78.6 120.5,78.6 C119.2,72.0 123.4,76.3 123.4,76.3 C127.3,80.9 125.5,87.3 125.5,87.3 C122.9,97.6 130.6,101.9 134.4,103.2"
				fill="currentColor"
				style="transform-origin: 130px 106px;"
				class="octo-arm" /><path
				d="M115.0,115.0 C114.9,115.1 118.7,116.5 119.8,115.4 L133.7,101.6 C136.9,99.2 139.9,98.4 142.2,98.6 C133.8,88.0 127.5,74.4 143.8,58.0 C148.5,53.4 154.0,51.2 159.7,51.0 C160.3,49.4 163.2,43.6 171.4,40.1 C171.4,40.1 176.1,42.5 178.8,56.2 C183.1,58.6 187.2,61.8 190.9,65.4 C194.5,69.0 197.7,73.2 200.1,77.6 C213.8,80.2 216.3,84.9 216.3,84.9 C212.7,93.1 206.9,96.0 205.4,96.6 C205.1,102.4 203.0,107.8 198.3,112.5 C181.9,128.9 168.3,122.5 157.7,114.1 C157.9,116.9 156.7,120.9 152.7,124.9 L141.0,136.5 C139.8,137.7 141.6,141.9 141.8,141.8 Z"
				fill="currentColor"
				class="octo-body" /></svg
		></a>
</div>

<style>
	.github-corner:hover .octo-arm {
		animation: octocat-wave 560ms ease-in-out;
	}
	@keyframes octocat-wave {
		0%,
		100% {
			transform: rotate(0);
		}
		20%,
		60% {
			transform: rotate(-25deg);
		}
		40%,
		80% {
			transform: rotate(10deg);
		}
	}
	@media (max-width: 500px) {
		.github-corner:hover .octo-arm {
			animation: none;
		}
		.github-corner .octo-arm {
			animation: octocat-wave 560ms ease-in-out;
		}
	}
</style>
